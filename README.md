...
- 欢迎图片支持**直接上传**（可多张，需 AstrBot >= v4.13.0），也支持填写网络 URL / 服务器本地路径 / `base64://` 数据。
- 内置一张默认欢迎图片：未配置任何图片时自动使用（可在配置中关闭）。
- 支持 `@` 新成员，提醒到对方。
...
| `image_file` | 欢迎图片（上传），可上传多张图片文件，需 AstrBot >= v4.13.0 |
| `image` | 欢迎图片（链接/路径），填写 `http(s)` 网络地址或服务器本地文件绝对路径，留空则不使用 |
| `use_default_image` | 当未配置上传图片和图片链接时，是否发送插件自带的默认欢迎图片，默认开启 |
